    ...
    def build_grid(self):
        ...
        for i in range(6):
            ...
            if i < len(self.discovered_apps):
                ...
                btn = Button(
                    rect=rect, text=title, style="APP", callback=make_launch_cb(path)
                )
            else:
                btn = Button(rect=rect, text="[EMPTY]", style="DISABLED", callback=None)
        ...
        self.buttons.append(None)  # Placeholder for disabled button
